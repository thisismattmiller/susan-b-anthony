...
export default {
  ...
  methods: {
    ...
    applyFacet(key,name){
        ...
        if (key === 'writtenFrom'){
            this.book.entries = this.book.entries.filter((v) => (v.cityOrState == name))

        }else if (key === 'people'){
            this.book.entries = this.book.entries.filter((v) => { if(!v.people){return false} return v.people.includes(name)} ) 
        }else if (key === 'locations'){
            this.book.entries = this.book.entries.filter((v) => { if(!v.geographicalLocations){return false} return v.geographicalLocations.includes(name)} )
        }
    ...
    async loadIndex(){
      ...
      this.$refs['date-selector'].scrollIntoView({ block: "start" })
    ...
    async loadBook(filename){
      ...
      this.activeFacet=false
      this.activeFacetValue=false
      let r = await fetch('/susan-b-anthony/json/'+filename)
    ...
    cleanFacetName (facet){
      ...
      facet = facet.replace('writtenFrom','From')
    ...
    expandEntry(event,daybook){


      let id = `${daybook.digital_id}_${daybook.file_id}_${daybook.entry_id}`
  ...
  watch:{
    ...
    daybookSelected(newVal,oldVal){

      let daybook = this.index.daybooks.filter((v)=>{ return (newVal === v.date) })[0]
      this.loadBook(daybook.file)
...
<div class="grid grid-cols-1 sm:grid-cols-10">
  ...
  <div class="col-span-2">
    <div class="">
      <h3 class="text-lg">Select Year</h3>

      <div class="w-full">
        <select size="10" v-model="daybookSelected" class="w-full rounded border-2  dark:border-0 dark:text-slate-800" ref="date-selector" >     
            <option v-for="(daybook, index) in index.daybooks" >{{daybook.date}}</option>
        </select>
      </div>
      <div v-if="activeFacet===false">
          ...
          <template v-for="(facet,key) in book.facets">
            ...
            <details>
                <summary class="cursor-pointer" v-if="key=='writtenFrom'">Facet by Location Written</summary>
                <summary class="cursor-pointer" v-if="key=='people'">Facet by People Mentioned</summary>
                <summary class="cursor-pointer" v-if="key=='locations'">Facet by Locations Mentioned</summary>
    ...
    <div class="p-2">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-10">
        <div class="col-span-8 pb-2 pr-2">
            <div class="mt-0 mb-2 text-2xl font-medium leading-tight">Susan B. Anthony - Daybooks</div>
            <div class="text-sm">Below is extracted data from Susan B. Anthony’s daybooks, housed at the Library of Congress. Click the summarized text or the triangle in each entry to expand to see more data and functionality. This <a href="https://thisismattmiller.com/post/using-gpt-on-library-collections/">blog post</a> describes how and why this data was created.</div>
      ...
      <table class="table-auto border-collapse">
        ...
        <tbody>
          <template v-for="(entry,index) in book.entries">
            ...
            <tr :class="['', { 'dark:bg-stone-750' : (index%2===0), 'bg-neutral-100 dark:bg-stone-900': (index%2==1)  }]">
              <td class="p-2" style="min-width: 8rem;">
                <span :class="['block',{'dark:bg-violet-700':showMLFields, 'bg-violet-300': showMLFields}]">{{entry.dateFormated}}</span>
                <!-- <span class="block" v-if="entry.cityOrState != ''">{{entry.cityOrState}}</span> -->
              </td>  
              <td class="p-2" style="max-width: 10rem;">
                <span  :class="['block',{'dark:bg-violet-700':showMLFields, 'bg-violet-300': showMLFields}]" v-if="entry.cityOrState != ''">{{entry.cityOrState}}</span>
              </td>  

              <td class="p-2 pt-4 pb-4 hover:cursor-pointer" :id="`${entry.digital_id}_${entry.file_id}_${entry.entry_id}_td`" v-on:click="expandEntry($event,entry)">
                <span data-ml="summarized" :class="['text-search',{'dark:bg-amber-700':showMLFields, 'bg-amber-300':showMLFields}]">{{entry.summaryText}}</span>
                <span data-ml="summarized" :class="['',{'dark:bg-amber-700':showMLFields, 'bg-amber-300':showMLFields}]" v-if="entry.summaryText == ''">No Entry</span>
              </td> 
              <td class="">
                <a href="#" v-on:click.prevent.stop="expandEntry($event,entry)">
                  <svg width="25px" height="25px" class="transition duration-250 ease-in-out" :id="`${entry.digital_id}_${entry.file_id}_${entry.entry_id}`" version="1.1" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
            ...
            <tr :class="['', { 'dark:bg-stone-750' : (index%2===0), 'bg-neutral-100 dark:bg-stone-900': (index%2==1)  }]">
              <td colspan=4>
                <Transition>
                  <div class="pt-4 grid grid-cols-4 gap-1" v-if="expand[`${entry.digital_id}_${entry.file_id}_${entry.entry_id}`]">
                      

                    <div class="col-span-1 text-center">
                        
                        <a class="m-auto block" :href="`https://www.loc.gov/resource/${entry.digital_id}/?sp=${entry.file_id}&st=text`" target="_blank"><img style="min-height: 215px;" class="" :src="entry.page"/></a>



<button v-if="entry.similar" @click="showSimilar(entry.similar,'daybook',entry.fullText)" class="bg-blue-200 mt-4 mb-4 text-sm hover:bg-blue-700 hover:text-white text-slate-600   font-bold py-2 px-2 rounded-full">
  View Similar Text
</button>
                    ...
                    <div class="p-1 col-span-3">
                      <code><pre :class="[{'dark:bg-violet-700':showMLFields, 'bg-violet-300': showMLFields}, 'text-search']" v-if="entry.fullText.includes('\n')">{{cleanText(entry.fullText)}}</pre><span class="text-search" v-else>{{cleanText(entry.fullText)}}</span></code>
                      <div class="grid grid-cols-2 pt-4">
                        <div class="col-span-1">
                          <span data-ml="extracted">People</span>
                          <ul class="list-disc pl-4">
                            <li v-for="p in entry.people.filter((v)=>{return (v.trim()!=='')})"><span :class="[{'dark:bg-violet-700':showMLFields, 'bg-violet-300': showMLFields}]">{{p}}</span></li>
                            <li class="italic" v-if="entry.people.filter((v)=>{return (v.trim()!=='')}).length==0">None</li>
                          </ul>
                        </div>
                        <div class="col-span-1">
                          <span data-ml="extracted">Places</span>
                          <ul class="list-disc pl-4">
                            <template v-if="entry.geographicalLocations && entry.geographicalLocations !== null">
                                <li v-for="p in entry.geographicalLocations.filter((v)=>{return (v.trim()!=='')})"><span :class="[{'dark:bg-violet-700':showMLFields, 'bg-violet-300': showMLFields}]">{{p}}</span></li>
                                <li class="italic" v-if="entry.geographicalLocations.filter((v)=>{return (v.trim()!=='')}).length==0">None</li>
                            </template>
                          </ul>
                        </div>
